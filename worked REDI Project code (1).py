...
ax.set_title('Continent Consumption TWH')
...
# Add tooltips to the bars with smaller font size and vertical orientation
for bar in bars:
    height = bar.get_height()
    ax.annotate(f'{height:,.0f}', xy=(bar.get_x() + bar.get_width() / 2, height),
                xytext=(0, 3), textcoords='offset points',
                ha='center', va='bottom', fontsize=8, rotation='vertical')
...
# Sort the dataframe by each power generation column and select the top 10 countries
...
if response.status_code == 200:
    ...
    # Sort the DataFrame by the founded year
    df.sort_values('Founded', inplace=True)

    # Plot the founded year against the number of employees as a line chart
    line = ax.plot(df['Founded'], df['Number of employees'])
    ax.set_xlabel('Founded Year')
    ax.set_ylabel('Number of employees')
    ax.set_title('Founded Year vs Number of employees')

    # Add tooltips to the data points
    for i, (x, y) in enumerate(zip(df['Founded'], df['Number of employees'])):
        if i % 5 == 0:  # Display tooltips for every 5th data point
            ax.annotate(f'{y}', (x, y), xytext=(5, -5), textcoords='offset points')
...
renewable_values = [250, 400, 300]  # Replace with actual renewable values
non_renewable_values = [600, 700, 800]  # Replace with actual non-renewable values

labels = renewable_industries + non_renewable_industries
values = renewable_values + non_renewable_values

# Calculate the total value for renewable and non-renewable energy categories
renewable_total = sum(renewable_values)
non_renewable_total = sum(non_renewable_values)

# Create a color map for the categories
colors = ['#FFD700', '#FFA500', '#FF6347', '#1E90FF', '#90EE90', '#FF1493']
